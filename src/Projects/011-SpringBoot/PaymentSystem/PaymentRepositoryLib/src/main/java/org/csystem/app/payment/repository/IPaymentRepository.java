...
public interface IPaymentRepository extends CrudRepository<Payment, Long> {
    ...
    //@Query("select p from Payment p inner join Customer c on c.username = p.customer.username where c.name = :name")
    //@Query("from Payment p inner join Customer c on c.username = p.customer.username where c.name = :name")
