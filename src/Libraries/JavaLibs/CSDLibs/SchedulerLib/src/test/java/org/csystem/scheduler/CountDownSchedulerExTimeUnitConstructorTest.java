...
public class CountDownSchedulerExTimeUnitConstructorTest {
    ...
    public CountDownSchedulerExTimeUnitConstructorTest(DataInfo dataInfo)
    {
        m_dataInfo = dataInfo;
    }
    ...
    {
        new CountDownSchedulerEx(m_dataInfo.durationInFuture, m_dataInfo.interval, m_dataInfo.timeUnit) {
            long value = m_dataInfo.durationInFuture;

            @Override
            protected void onStart()
            {
                Assert.assertEquals(m_dataInfo.durationInFuture, value);
